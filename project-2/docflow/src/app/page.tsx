              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-neutral-500">
                    {formatLabel(getRecordString(job, "doc_type"))} · #
                    {String(job.id)}
                  </p>
                </div>
                <span
                  className={`shrink-0 rounded border px-2 py-1 text-xs font-semibold ${statusTone(job)}`}
                >
                  {humanState(job)}
                </span>
              </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-3 rounded border border-neutral-200 bg-white p-5">
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  onClick={() =>
                    callRoute("GET /jobs/exceptions", "/jobs/exceptions")
                  }
                  disabled={loading !== ""}
                  className="rounded border border-neutral-300 px-3 py-2 text-sm font-semibold hover:bg-neutral-100 disabled:cursor-not-allowed disabled:opacity-60"
                >
            ...
            <JobList
              ...
              jobs={reviewJobs}
              onSelect={(job) =>
                setResult({ title: "Selected Job", data: job })
              }
            />
            <JobList
              title="Completed"
          ...
          <section className="flex flex-col gap-4">
            <div className="rounded border border-neutral-200 bg-white p-5">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                {selectedRecord && (
                  <span
                    className={`w-fit rounded border px-3 py-1.5 text-sm font-semibold ${statusTone(selectedRecord)}`}
                  >
